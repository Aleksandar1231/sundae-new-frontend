 ...
 * @dev and your consumer contract as a consumer of it (see VRFCoordinatorInterfaceFUDGE
...
contract Lotto is ILotto, VRFConsumerBaseV2 {
    ...
    address public immutable STRAW;

    // Burn GameFi Amounts
    uint256 public FUDGES_BURNED_PER_TEN_TICKETS = 50 * 10**18;
    ...
    // avax coordinator
    address private constant vrfCoordinator = 0xd5D517aBE5cF79B7e95eC98dB0f0277788aFF634; //need premium subscription
    ...
    /**
        Sets The Number Of STRAW To Burn To Create Ten Tickets For The User
        @param burnedPerTen number of STRAW, cannot be zero
     */
    function setFUDGESBurnedPerTenTickets(uint burnedPerTen) external onlyOwner {
        require(
            burnedPerTen > 0,
            'Cannot Be Zero'
        );

        FUDGES_BURNED_PER_TEN_TICKETS = burnedPerTen;
    }
    ...
    /**
        Burns STRAW For Tickets
        The amount transferred in depends on FUDGES_BURNED_PER_TEN_TICKETS
        NOTE: Must Have Prior Approval of STRAW for address(this) before calling
        @param batches - number of batches of ten tickets to mint
     */
    function burnSTRAW(uint batches) external {
        // transfer in STRAW
        uint received = _transferIn(IERC20(STRAW), FUDGES_BURNED_PER_TEN_TICKETS * batches);
        require(
            received == FUDGES_BURNED_PER_TEN_TICKETS * batches,
            'Invalid Tokens Received'
        );
        // burn STRAW
        IERC20(STRAW).transfer(treasury, received);
