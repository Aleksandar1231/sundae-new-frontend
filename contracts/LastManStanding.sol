...
contract TournamentERC20 is Ownable, ITournament {
    ...
    uint256 lmsPotSize;
    uint256 lmsPotClaimed;

    constructor (address _token, uint256 _periodSeconds, uint256 _roundSeconds, uint256 _breakevenSeconds, uint256 _ticketSize, uint256 _potShare, uint256 _burnShare, address _treasury) {
        ...
        require(POT_SHARE + BURN_SHARE <= 10000);
    ...
    function _getPotSize() internal view returns (uint256) {
        ...
        return rewardToken.balanceOf(address(this)) + lmsPotClaimed - lmsPotSize;
    }

    function getLmsPotSize() public view returns (uint256) {
        return lmsPotSize;
    }
    ...
    function buy() external payable {
        ...
        if (10000 - POT_SHARE - BURN_SHARE > 0) {
            rewardToken.transfer(owner(), transferSize * (10000 - POT_SHARE - BURN_SHARE) / 10000);
        }
        winner = msg.sender;
    }
